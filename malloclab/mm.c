 ...
 * 获取大小为size的内存位置，如果size==0直接返回NULL
 * 首先需要将size进行8字节对其扩充位asize，如果需要的字节大小小于等于8字节
 * 根据块大小最低是16字节，asize应该是16字节，如果超过了8字节，需要根据请求的字节数+头+尾向上扩充到8字节的倍数
 * 然后找到合适的块大小，如果找到了直接对应的指针直接分，调用place进行对空闲块修改位已分配的块
 * 如果没有找到相应的块大小，那么会将当前的asize对比，当容量不够的时候自动扩充内存的最大大小值取最大的
 * 因为是二者取最大的，必定可以分配成功
 */
 ...
 * 首先调用malloc函数先获取一定大小的内存
 * 然后获取之前的内存块的大小，如果新的大小比原来的还小
 * 那么仅仅是将原来内存的低地址copy一边
 * 如果新的size大，那么将原来的内存大小全部复制到新的地方
 * 调用memcpy进行每个字节复制
 * 使用free函数进行释放空间
 */
