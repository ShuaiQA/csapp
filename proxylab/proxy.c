...
void do_it(int fd)
{
    ...
    sprintf(buf_server, "GET %s HTTP/1.0\r\nHost: %s\r\n%s%s%s%s%s%s%s\r\n",
            path, host, user_agent_hdr, accept_hdr, language_hdr,
            accept_encoding_hdr, connection_hdr, secure_hdr, pxy_connection_hdr);
    ...
    if(content_len<MAX_OBJ){
        printf("content too big not store cache");
